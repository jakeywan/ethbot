...
export const maticUSDCBot = async () => {

  console.log('Listening to MATIC-USDC pools...')
  ...
  const loadPairs = async () => {
    sushiEthUSDC = await loadPair(maticAddress, usdcAddress, 'SUSHI')
    quickSwapEthUSDC = await loadPair(maticAddress, usdcAddress, 'QUICKSWAP')
    polyzapEthUSDC = await loadPair(maticAddress, usdcAddress, 'POLYZAP')
  }
  ...
  provider.on('block', async (blockNumber) => {
    try {
      ...
      const reserve1Sushi = Number(ethers.utils.formatUnits(sushiReserves[1], usdcDecimals))
      ...
      const reserve1QuickSwap = Number(ethers.utils.formatUnits(quickSwapReserves[1], usdcDecimals))
      ...
      const reserve1Polyzap = Number(ethers.utils.formatUnits(polyzapReserves[1], usdcDecimals))
      ...
      console.log('QUICKSWAP PRICE               =>', parseFloat(priceQuickSwap.toFixed(6)))
      console.log('SUSHISWAP PRICE               =>', parseFloat(priceSushiswap.toFixed(6)))
      console.log('POLYZAP PRICE                 =>', parseFloat(pricePolyzap.toFixed(6)))
