/**
 * NOTES
 * The pool on Dyfn isn't working for some reason.
 */
...
export const maticBtcBot = async () => {

  console.log('Listening to MATIC-BTC pools...')
  ...
  const loadPairs = async () => {
    sushiEthUSDC = await loadPair(maticAddress, btcAddress, 'SUSHI')
    quickSwapEthUSDC = await loadPair(maticAddress, btcAddress, 'QUICKSWAP')
    polyzapEthUSDC = await loadPair(maticAddress, btcAddress, 'POLYZAP')
    dfynEthUSDC = await loadPair(maticAddress, btcAddress, 'DFYN')
  }
  ...
  provider.on('block', async (blockNumber) => {
    try {
      ...
      const reserve1Sushi = Number(ethers.utils.formatUnits(sushiReserves[1], btcDecimals))
      ...
      const reserve1QuickSwap = Number(ethers.utils.formatUnits(quickSwapReserves[1], btcDecimals))
      ...
      const reserve1Polyzap = Number(ethers.utils.formatUnits(polyzapReserves[1], btcDecimals))
      const pricePolyzap = reserve0Polyzap / reserve1Polyzap

      /**
       * Dfyn prices
       * https://info.dfyn.network/pair/0x3318801171f0705781ab0f97ebccef73b2486165
       */
      // const dfynReserves = await dfynEthUSDC.getReserves()
      // const reserve0Dfyn = Number(ethers.utils.formatUnits(dfynReserves[0], maticDecimals))
      // const reserve1Dfyn = Number(ethers.utils.formatUnits(dfynReserves[1], btcDecimals))
