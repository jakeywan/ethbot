...
export const ethUSDCBot = async () => {

  console.log('Listening to ETH-USDC pools...')
  ...
  const loadPairs = async () => {
    sushiEthUSDC = await loadPair(usdcAddress, wethAddress, 'SUSHI')
    quickSwapEthUSDC = await loadPair(usdcAddress, wethAddress, 'QUICKSWAP')
    polyzapEthUSDC = await loadPair(usdcAddress, wethAddress, 'POLYZAP')
    dfynEthUSDC = await loadPair(usdcAddress, wethAddress, 'DFYN')
  }
  ...
  provider.on('block', async (blockNumber) => {
    ...
    const reserve0Sushi = Number(ethers.utils.formatUnits(sushiReserves[0], usdcDecimals))
    ...
    const reserve0QuickSwap = Number(ethers.utils.formatUnits(quickSwapReserves[0], usdcDecimals))
    ...
    const reserve0Polyzap = Number(ethers.utils.formatUnits(polyzapReserves[0], usdcDecimals))
    ...
    const reserve0Dfyn = Number(ethers.utils.formatUnits(dfynReserves[0], usdcDecimals))
